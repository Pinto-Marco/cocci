...
class AddToCartView(APIView):
    ...
    def post(self, request):
        if request.data:
            ...
            return Response(
                {"error": "Missing product_code or quantity"},
...
class CheckoutView(APIView):
    ...
    def post(self, request):
        ...
        # Create order
        order = Order.objects.create(email=email, session_id=session_id, total=total)
...
def ProductDetailView(request, code):
    ...
    context = {"product": product, "images": [image.image for image in images]}
